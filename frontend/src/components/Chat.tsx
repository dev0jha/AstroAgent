...
export const Chat: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden relative z-10">
      {/* Chat Header */}
      <header className="h-16 border-b border-border bg-card/70 backdrop-blur-md px-6 flex items-center justify-between select-none shrink-0">
          ...
          {messages.length === 0 ? (
            <div className="my-auto text-center max-w-lg mx-auto py-12 px-4 animate-fade-in select-none">
              ...
              <button
                ...
                className="bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl px-6 py-3 text-sm font-bold tracking-wider uppercase shadow-xl shadow-primary/15 transition-all duration-200"
              >
                Cast Your Chart
              </button>
            </div>
      ...
      <footer className="border-t border-border bg-card/45 backdrop-blur-md p-4 sm:p-6 shrink-0">
        <div className="max-w-3xl mx-auto flex items-end gap-3 relative">
          <textarea
            ...
            className="flex-1 bg-input border border-border rounded-xl px-4 py-3 outline-none text-sm sm:text-base text-foreground resize-none placeholder:text-muted-foreground/40 font-sans focus:border-primary focus:ring-1 focus:ring-primary/30 min-h-[46px] max-h-[120px] transition-all duration-200"
          />
          <button
            onClick={handleSend}
            disabled={!input.trim() || isStreaming}
            className="bg-primary hover:bg-primary/90 disabled:opacity-40 disabled:hover:bg-primary text-primary-foreground w-11 h-11 flex items-center justify-center rounded-xl shadow-lg shadow-primary/10 transition-all duration-200 flex-shrink-0 cursor-pointer disabled:cursor-not-allowed"
          >
